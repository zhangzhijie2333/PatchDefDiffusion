...
class GroundingNetInput:
    ...
    def prepare(self, batch):
        ...
        self.batch, self.C, self.H, self.W = edge.shape
        self.device = edge.device
        self.dtype = edge.dtype

        return {"edge":edge, "mask":mask}
    ...
    def get_null_input(self, batch=None, device=None, dtype=None):
        ...
        edge = th.zeros(self.batch, self.C, self.H, self.W).type(dtype).to(device)
