...
class PositionNet(nn.Module):
    ...
    def forward(self, edge, mask):
        ...
        # token from edge map 
        edge = torch.nn.functional.interpolate(edge, self.resize_input, mode="nearest")
        edge = self.in_conv(edge)
        edge_feature = self.convnext_tiny_backbone(edge)
        objs = edge_feature.reshape(B, -1, self.num_tokens)
