...
def create_expt_folder_with_auto_resuming(OUTPUT_ROOT, name, enable_resume=True, resume_tag=None):
    ...
    if os.path.exists(name):
        ...
        if enable_resume:
            if resume_tag is not None:
                potential_ckpt = os.path.join(name, resume_tag, 'edge_checkpoint_latest.pth')
                if os.path.exists(potential_ckpt):
                    checkpoint = potential_ckpt
                    if get_rank() == 0:
                        print('Load checkpoint from the specified tag:' + potential_ckpt)
                else:
                    raise FileNotFoundError(f'The specified tag {resume_tag} was not found edge_checkpoint_latest.pth')
            else:
                for previous_tag in all_existing_tags:
                    potential_ckpt = os.path.join(name, previous_tag, 'edge_checkpoint_latest.pth')
                    if os.path.exists(potential_ckpt):
                        checkpoint = potential_ckpt
                        if get_rank() == 0:
                            print('Found automatically recovered checkpoint:' + potential_ckpt)
                        break
        
        idx = 0
        while True:
            curr_tag = 'tag' + str(idx).zfill(2)
            candidate_path = os.path.join(name, curr_tag)
...
class Trainer:
    def __init__(self, config):
        ...
        self.name, self.writer, checkpoint = create_expt_folder_with_auto_resuming(config.OUTPUT_ROOT, config.name,enable_resume=False,)
        if get_rank() == 0:
        ...
        state_dict = read_official_ckpt('./sd-v1-4.ckpt')
        ...
        if self.config.ckpt is not None:
            first_stage_ckpt = torch.load(self.config.ckpt, map_location="cpu", weights_only=False)
            loaded_state_dict = first_stage_ckpt["model"]
        ...
        trainable_names = [name for name, _ in self.model.named_parameters()] + \
                          [f"autoencoder.{name}" for name, _ in self.autoencoder.named_parameters()]
        all_params_name = trainable_names
        ...
        if checkpoint is not None:
            ...
            if checkpoint is not None and "opt" in checkpoint:
                try:
                    ...
                    print("⚠️ Warning: Optimizer state not loaded due to mismatch.")
                    print("Reason:", e)
    ...
    def save_ckpt_and_result(self):
        ...
        torch.save( ckpt, os.path.join(self.name, "edge_checkpoint_latest.pth") )
